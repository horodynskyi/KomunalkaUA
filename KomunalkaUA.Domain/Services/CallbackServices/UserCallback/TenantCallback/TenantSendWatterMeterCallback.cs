...
public class TenantSendWatterMeterCallback:ITenantSendWatterMeterCallback
{
    private int _tenantId;
    private string _callback ="tenant-send-watter-meter";
    private readonly IRepository<FlatMeter> _flatMeterRepository;
    private readonly IRepository<State> _stateRepository;

    public TenantSendWatterMeterCallback(
    ...
    {
        var flatMeter = await _flatMeterRepository.GetBySpecAsync(new FlatMeterGetByTenantIdIncludeMeterFlatSpec(_tenantId,(int) MeterTypeEnum.Watter));
        ...
        await client.TryEditMessageTextAsync(
            ...
            $"Введіть поточні показник води (попередні - {flatMeter.Meter.Value}):",
