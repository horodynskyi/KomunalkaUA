...
public class TenantSendElectricalMeterCallback:ITenantSendElectricalMeterCallback
{
    private int _tenantId;
    private string _callback ="tenant-send-electrical-meter";
    private readonly IRepository<FlatMeter> _flatMeterRepository;
    private readonly IRepository<State> _stateRepository;

    public TenantSendElectricalMeterCallback(
    ...
    {
        var flatMeter = await _flatMeterRepository.GetBySpecAsync(new FlatMeterGetByTenantIdIncludeMeterFlatSpec(_tenantId,(int) MeterTypeEnum.Electric));
        ...
        await client.TryEditMessageTextAsync(
            ...
            $"Введіть поточний показник світла (попередній - {flatMeter.Meter.Value}):",
