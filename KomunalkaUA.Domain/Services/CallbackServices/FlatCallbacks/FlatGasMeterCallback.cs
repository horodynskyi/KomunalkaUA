...
public class FlatGasMeterCallback:IFlatGasMeterCallback
{
    private readonly string _callback = "flat-gas-meter";
    private int _flatId;
    ...
    public async Task ExecuteAsync(CallbackQuery callbackQuery, ITelegramBotClient client)
    {
        ...
        if (meters.Any(x => x.Meter.Provider.MeterTypeId == (int?) MeterTypeEnum.Gas && x.FlatId==_flatId))
        {
            var meter = meters.FirstOrDefault(x => x.Meter.Provider.MeterTypeId == (int?) MeterTypeEnum.Gas && x.FlatId==_flatId);
        ...
        var providers = await _providerRepository.ListAsync(new ProviderGetByCityIdAndMeterTypeIdSpec((int)flat.Address.CityId,(int)MeterTypeEnum.Gas));
        await client.EditMessageTextAsync(
            callbackQuery.From.Id,
            callbackQuery.Message.MessageId,
            "Виберіть зі списку поставщика газу",
