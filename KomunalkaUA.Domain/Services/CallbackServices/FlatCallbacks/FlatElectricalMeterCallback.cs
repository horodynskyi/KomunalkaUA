...
public class FlatElectricalMeterCallback:IFlatElectricalMeterCallback
{
    private readonly string _callback = "flat-electrical-meter";
    private int _flatId;
    ...
    public FlatElectricalMeterCallback(
    ...
    public async Task ExecuteAsync(CallbackQuery callbackQuery, ITelegramBotClient client)
    {
        ...
        if (meters.Any(x => x.Meter.Provider.MeterTypeId == (int?) MeterTypeEnum.Electric && x.FlatId==_flatId))
        {
            var meter = meters.FirstOrDefault(x => x.Meter.Provider.MeterTypeId == (int?) MeterTypeEnum.Electric && x.FlatId==_flatId);
            await client.TryEditMessageTextAsync(
                ...
                $"\n<b>Показник:</b>:<code> {meter.Meter.Value??0} kw</code>",
        ...
        var providers = await _providerRepository.ListAsync(new ProviderGetByCityIdAndMeterTypeIdSpec((int)flat.Address.CityId,(int)MeterTypeEnum.Electric));
        await client.EditMessageTextAsync(
            callbackQuery.From.Id,
            callbackQuery.Message.MessageId,
            "Виберіть зі списку постачальника електроенергії:",
